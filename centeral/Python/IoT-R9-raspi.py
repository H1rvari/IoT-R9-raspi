    ...
    while True:
        ...
        try:
            async with BleakClient(device) as client:
                ...
                """"
                The sensor is connected first and then the same function is called recursively
                to connect the remote. After that the program stays in an infinite loop (unitl disconnected).
                Afther the function is called again, there is no way to detecct if the sesor is still connected.
                This is very clunky and bad way to do this but it's the only way we got it to work, since we had
                problems with executing the functions in parallel on rasberry pi.
                """
                if name == "SENSOR":
